...
!=======================================================================
       !! ALLOCATE THE 1 HOTSTART VARIABLE
       JMT_LOCAL= 1

      WRITE(IPT,*)'ICE VARIABLES SIZE'
      WRITE(IPT,*)IMT_LOCAL,JMT_LOCAL,NCAT,NARR
      ALLOCATE(AICEN(M,JMT_LOCAL,NCAT));AICEN=0.0_SP  ! CONCENTRATION OF 1
      ALLOCATE(VICEN(M,JMT_LOCAL,NCAT));VICEN=0.0_SP  ! VOLUME PER UNIT AREA OF 1          (M)
      ALLOCATE(VSNON(M,JMT_LOCAL,NCAT));VSNON=0.0_SP  ! VOLUME PER UNIT AREA OF SNOW         (M)
      ALLOCATE(TSFCN(M,JMT_LOCAL,NCAT));TSFCN=0.0_SP  ! TEMPERATURE OF 1/SNOW TOP SURFACE  (C)
      ...
      !-----------------------------------------------------------------
      ! OTHER VARIABLES CLOSELY RELATED TO THE STATE OF THE 1
      !-----------------------------------------------------------------
      ALLOCATE(STRENGTH(M,JMT_LOCAL)); STRENGTH=0.0_SP ! 1 STRENGTH      

      ALLOCATE( FRESH  (1:M,1)); FRESH=0.0_SP    ! FRESH WATER FLUX TO OCEAN (KG/M^2/S)
      ALLOCATE( FSALT  (1:M,1)); FSLAT=0.0_SP    ! SALT FLUX TO OCEAN (KG/M^2/S)
      ...
      ALLOCATE(UICE2(0:N))      ; UICE2 = 0.0_SP ! 1 VELOCITY
      ALLOCATE(VICE2(0:N))      ; VICE2 = 0.0_SP ! 1 VELOCITY

      ALLOCATE(SIG1(0:M))      ;  SIG1 = 0.0_SP  ! 1 STRESS TENSOR
      ALLOCATE(SIG2(0:M))      ;  SIG2 = 0.0_SP  ! 1 STRESS TENSOR 
      ALLOCATE(SIG12(0:M))     ; SIG12 = 0.0_SP  ! 1 STRESS TENSOR
