...
%
%Altered by J. Pringle so that if z is (M,length(x)), then the output is
%(M,length(xi)), and interpolation is done seperately on each row 
%of z.  This should be much faster, because redundant triangulation, etc
%is eliminated.   Only linear interpolation is implemented.
%
%
...
%   $Revision: 5.28 $  $Date: 2001/04/15 11:59:14 $

error(nargchk(5,6,nargin))

%JMP-create a dummy z, which has only one row of the input z, so that
%JMP I can use the code below largely unchanged.  The orginal z goes
%JMP into zfull, and z becomes a vector of 1:length(x), which is used
%JMP below to keep track of how the routine reshuffles the elements of x, y
%JMP and z
 zfull=z;
 z=1:size(zfull,2);
...
%------------------------------------------------------------
function zi = linear(x,y,z,xi,yi,zfull)
...
%JMP do interpolation over all rows of zfull, and save output 
%JMP in zi

zorder=z;
zi=zeros(size(zfull,1),length(xi));
for nk=1:size(zfull,1)
  z = zfull(nk,zorder);
